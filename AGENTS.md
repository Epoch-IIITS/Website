...
- Event timestamps are displayed in India Standard Time through the shared `lib/event-dates.ts` formatters and explicit timezone-aware ticket formatting. Do not use browser/server-local date formatting for event timestamps.
...
- Gallery listings are ordered by event date newest first, with creation time and ID as stable tie breakers. Each public gallery card overlays its event name, description, date, and photo count on its selected cover image. `Gallery.coverImage` points to one of the gallery image URLs; legacy records without it fall back to the first image. New and edit forms let admins change the cover without changing photo order.
...
- Successful administrator creates, updates, and deletes, along with member-initiated updates to linked team profiles, are recorded atomically with their domain writes through `lib/audit-log.ts`. The admin-only `/admin/logs` page provides searchable, filterable activity history. Member profile logs identify the signed-in member as actor and include only changed public profile fields, excluding the private account email and user link. Audit values are allowlisted, large blog content and gallery image changes are summarized, and sensitive contact/QR payloads are not copied. Compound team changes share an operation ID; event and QR deletions include cascade counts. `AuditLog` records are immutable and expire automatically six calendar months after creation through a TTL index; the read API also excludes expired records while MongoDB's asynchronous TTL cleanup catches up. Audited mutations require a MongoDB replica set because the domain and audit records share a transaction.
...
- `models/Team.ts` contains `TeamYear`, `TeamPerson`, `TeamAppointment`, `TeamRequest`, and `TeamSettings`. Profiles hold current career details and may store a private account email and user ID for self-service editing; neither identifier is returned publicly. Appointments hold historical AY/POR/group/order. A person has at most one appointment per AY. The singleton settings record stores the current year, so only one can be current. Switch the current year before unpublishing it.
...
- `/team/request` requires sign-in and publication consent. `/api/team/requests` records the session's identity, allows one pending request per applicant/year, and returns only that applicant's request status/feedback. Admins can approve or reject with feedback. Approval links the applicant account to one person profile, reusing an existing account link for later years. Existing profiles are preserved when linking a request.
- `/profile` shows a Team directory profile editor only when the signed-in user ID or normalized account email matches a `TeamPerson` with at least one appointment, or an older approved request resolves that person through its saved appointment. Members may update the shared name, photo, LinkedIn URL, current role, organization/institute, and tagline through `/api/team/profile`; appointments, PORs, years, order, and publication remain admin-only. The People editor's private Account email field enables this for manually created profiles. Unmatched accounts see an explicit explanation instead of a hidden editor.
- All image uploads use a shared 4 MB per-file limit in `lib/image-upload.ts`, validated before sending and again by the APIs. The limit leaves room for multipart overhead below the hosting request-body limit. The shared client helper handles non-JSON hosting errors, expired sessions, network/server failures, and malformed success responses. Gallery batches retain successful uploads and list failed filenames with reasons for retry.
- `/api/team/upload` is an authenticated Cloudinary photo upload endpoint for JPG/PNG/WebP up to 4 MB. New team photos use the `epoch/team` folder; existing `epoch-team` links remain supported. It is separate from the content upload endpoint.
